...
from model import get_boulder_models, get_route_models, test_model_on_hold
...
def split_data_ability(df,col, sample_num):
    ...
    X_hold = test[test['ascent_date_year'] != 2017].sample(sample_num)
...
def main_ability_boulder():
    ...
    est_model_on_hold(X_train,y_train,X_hold,y_hold,Ridge,boulder_ridge.alpha_,'Final Ridge Prediction for Boulder grades','images/ridge_model_boulder_ability.png')
    test_model_on_hold(X_train,y_train,X_hold,y_hold,Lasso,boulder_lasso.alpha_,'Final Lasso Prediction for Boulder grades','images/lasso_model_boulder_ability.png')

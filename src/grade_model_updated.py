...
from sklearn.linear_model import RidgeClassifier, Lasso, RidgeClassifierCV, LassoCV
...
def run_ridge_model(X,y):
    ...
    ridge = RidgeClassifierCV(alphas = np.logspace(-2,4,num=250),cv=10)
    ridge.fit(X_train_std,y_train)
    y_hats = ridge.predict(X_test_std)
    ridge_score = r2_score(y_test,y_hats)
...
def test_model_on_hold(X_train,y_train,X_hold,y_hold,model,alpha,title,filename):
    ...
    standardizer.fit(X_train)
    X_train_std = standardizer.transform(X_train)
    X_hold_std = standardizer.transform(X_hold)
    final_model = model(alpha)
    final_model.fit(X_train_std,y_train)
    y_pred = final_model.predict(X_hold_std)
    plot_model_predictions(y_hold, y_pred, title, filename)
    final_score = final_model.score(X_hold_std,y_hold)
...
def main_boulder(df,title1,filename1):
    boulder = drop_columns_grades(df)
    X_train, y_train, X_hold, y_hold = split_data_grades(boulder,'usa_boulders')
    boulder_ridge = get_boulder_models(X_train, y_train)
    test_model_on_hold(X_train,y_train,X_hold,y_hold,RidgeClassifier,boulder_ridge.alpha_,title1,filename1)
    boulder_coefs = get_coefs(boulder_ridge,boulder.drop('usa_boulders',axis=1))
    print(boulder_coefs.sort_values(0))

def main_route(df,title1,filename1):
    route = drop_columns_grades(df)
    X_train, y_train, X_hold, y_hold = split_data_grades(route,'usa_routes')
    route_ridge = get_route_models(X_train,y_train)
...
def restrict_boulder():
    ...
    main_boulder(boulders_restrict,'Final Ridge Prediction for Boulder grades','images/ridge_model_boulder_restrict.png')

def restrict_route():
    boulders, routes = import_data()
    routes_restrict = routes.copy()
    routes_restrict = routes_restrict.loc[routes_restrict['usa_routes'] >= 5.11]
    main_route(routes_restrict,'Final Ridge Prediction for Route grades','images/ridge_model_route_restrict.png')
...
def add_features_model_boulder():
    ...
    main_boulder(boulder,'Ridge Prediction for Boulder grades','images/ridge_model_boulder_add.png')

def add_features_model_route():
    boulders, routes = import_data()
    route = add_features(routes)
    main_route(route,'Ridge Prediction for Route grades','images/ridge_model_route_add.png')

def run_all_boulder_models():
    boulders, routes = import_data()
    main_boulder(boulders,'Ridge Prediction for Boulder grades','images/ridge_model_boulder_final.png')
    restrict_boulder()
    add_features_model_boulder()

def run_all_route_models():
    boulders, routes = import_data()
    main_route(routes,'Ridge Prediction for Route grades','images/ridge_model_route_final.png')
    restrict_route()
...
def edit_route_grades():
    ...
    main_route(routes,'Ridge Prediction for Route grades','images/ridge_model_route_updated.png')
